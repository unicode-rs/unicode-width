...
LOOKUP_PATH = "../src/gen/lookup.rs"
"""The path of the emitted generated lookup functions (relative to the working directory)"""
...
FILE_HEADER = """// Copyright 2012-2025 The Rust Project Developers. See the COPYRIGHT
// file at the top-level directory of this distribution and at
// http://rust-lang.org/COPYRIGHT.
//
// Licensed under the Apache License, Version 2.0 <LICENSE-APACHE or
...
def emit_rust_file(path: str, generator: Callable[[IO[str]], None]):
    """Outputs a Rust file to `path` using the provided `generator` function.
    Automatically adds a license header.
    """
...
def lookup_fns(
    ...
    if is_cjk:
        ...
        cjk_cap = ""
        ambig = "narrow"
    s = f"""/// Returns the [UAX #11](https://www.unicode.org/reports/tr11/) based width of `c` by
...
def emit_width_info(module: IO[str]):
    """Outputs the WidthInfo autogenerated constants."""
    module.write(
        """use super::WidthInfo;
...
"""
    ...
    for i, table in enumerate(tables):
        ...
        if table.bytes_per_row is None:
            ...
            module.write(
                f"pub(crate) static {table.name}: Align{table.align}<[u8; {len(byte_array)}]> = Align{table.align}(["
            )
            ...
            module.write(
                f"""
#[cfg(feature = "cjk")]
pub(crate) const {table.name}_LEN: usize = {num_rows};
#[cfg(not(feature = "cjk"))]
pub(crate) const {table.name}_LEN: usize = {num_primary_rows};
/// Autogenerated. {subtable_count} sub-table(s). Consult [`lookup_width`] for layout info.
pub(crate) static {table.name}: Align{table.align}<[[u8; {table.bytes_per_row}]; {table.name}_LEN]> = Align{table.align}([\n"""
            )
...
def emit_tests(
    ...
    """Outputs a Rust module to `module` containing tests for character width.
    """
...
#[cfg(feature = "cjk")]
pub(crate) static NORMALIZATION_TEST_WIDTHS_CJK: [WidthInfo; {len(test_width_variants_cjk) + 1}] = [
    WidthInfo::DEFAULT,
"""
    )
...
#[rustfmt::skip]
pub(crate) static NORMALIZATION_TEST: [(&str, &str, &str, &str, &str); {len(normalization_tests)}] = [
"""
    )
